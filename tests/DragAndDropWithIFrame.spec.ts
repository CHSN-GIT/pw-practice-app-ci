...
// import { test, expect } from "@playwright/test";  // esto lo redifiniremos para poder importar nuestra variable globalsQaURL
import { expect } from "@playwright/test";  // desde playwright solo importaremos expect ya que test lo traeremos desde test-options
                                            // (que es una extención de test) ya que así incorporaremos nuestra variable 
import { test } from '../test-options'


// Ahora el nuevo test apuntando a la dirección web con un ejemplo https://globalsqa.com/demo-site/draganddrop/

// test('Drag and Drop with Iframe', async({page}) => {  // tambien redefiniremos este test para que incluya como parámetro nuestra
                                                      // variable globalsQaURL

    test('Drag and Drop with Iframe', async({page, globalsQaURL}) => {

    // await page.goto('https://globalsqa.com/demo-site/draganddrop/')  // ya podremos reemplazar este link a fuego por nuestra variable
